...
* UVA-221 离散化启蒙题
* UVA-10391 如果写字典树实现的话，是个不错的基础练习。
* UVA-1596 递归，词法解析
* UVA-1597 多种容器使用的基础练习。
* UVA-12333 大数+字典树，感觉应该放在基础数据结构部分。
...
思路：队列模拟就行了，题目有个坑是没说只有1张牌的时候怎么输出。[code](./uva10935.cpp)

### UVA-10763 Foreign Exchange

题意: 给N组数对，<a,b> 与 <b,a> 构成一对，问是否存在完美匹配。

思路： 统计 < a,b >,其中 a < b。的数量是否为偶数。 [code](./uva10763.cpp)


### UVA-10391 Compound Words

题意： 按字典序给一个单词集合，问是否有单词是另外两个单词的组合，按字典序输出符合条件的单词

思路：

* 用set来做，set保存所有的单词，枚举所有单词，对于每个单词枚举其前缀和后缀，判断是否在set中存在。
题目说有12w的单词量，感觉做起来应该比较慢，但是20ms就跑完了，感觉数据比较弱。 [code](./uva10391.cpp)

* 用字典树。先把单词建树，然后再依次枚举单词进行匹配。如果单词的某个字符匹配到终止状态，就转到根匹配剩余的字符，如果最终达到终止状态，那么就输出；否则继续匹配。
其实也是进行前后缀的匹配，只是这样做应该比用set来做更快点，如果数据足够多的话。但是写起来要麻烦很多。[code](./uva10391-1.cpp)
...
思路：按`< y , x >`排序。对于相同的`y`，求是否成镜像（首尾对应点`x`坐标平均值一致）及中线`x`坐标，判断所有这样的y是否成镜像且中线坐标相同。注意所有`x*2`。 [code](./uva1595.cpp)


### UVA-12100 Printer Queue 

题意：一个打印队列，如果队列中有优先级更高的任务，就把队头的任务放到队尾去，问你的任务什么时候能打印出来。

思路：记录所有优先级任务的数量，维护当前最高优先级，队列模拟。 [code](./uva12100.cpp)

### UVA-230 Borrowers

题意: 书按作者，书名排列，借书、还书、整理三种命令，输出整理命令产生的整理操作。

思路：输入稍微麻烦点，为了简化代码，用了正则。记录书的状态（在架，借出，还回），按顺序枚举操作即可。注意前面的书都借出时的整理操作。[code](./uva230.cpp)

### UVA-1596 Bug Hunt

题意： 给一个脚本的BNF，要你写一个程序找第一次出现bug的位置。

思路：BNF给的比较简单，只涉及到数组的声明、赋值和引用，直接写个递归下降的parse即可。 [code](./uva1596.cpp)


### UVA-1597 Searching the Web

题意： n个文档，m次查询，查找文档的单词所在行。 查询分为

+ 单词，输出所有含这个单词的文档的含单词行
+ 单词1 AND 单词2，输出所有含这两个单词的文档的含单词行
+ 单词1 OR 单词，输出所有含其中一个单词的文档的含单词行
+ NOT 单词，输出所有不含这个单词的文档的所有行

 (莫名RE，挂机中)

### UVA-12504 Updating a Dictionary 

题意：有两个字典，问增加，删除和更新了哪些条目。

思路：建两个字典，然后类似于归并，处理一下就可以了。用正则解析输入方便得很。[code](./uva12504.cpp)

### UVA-511 Do You Know the Way to San Jose?

题意：给若干地图（矩形），若干点，然后若干次查询，每次查询询问一个点的第i级地图是哪个？
级别排序：面积大，中心点距离大，比例距离0.75大，右下端点距离小，左下x大

最坑的细节是：题目对于地图级别的定义有歧义，题目之前说地图按面积排序确定级别，最后一段又说是对于包含点的地图中，按面积等规则进行排序的级别；
而我之前理解以为是所有地图按面积进行排序。

自己把`detail`打成`detial`!!!，嘤嘤嘤。

就是个阅读理解题，坑！ [code](./uva511.cpp)


### UVA-12333 Revenge of Fibonacci

题意：求给定前缀的最小Fibonacci项。

坑： < 100000 项，不是 <=

思路： 大数算fib，建立字典树，查询前缀。 由于前缀最长40，只要针对最多前40个字符建立trie即可。 [code](./uva12333.cpp)

还能优化fib大数计算，其实完全不需要算这么长，只要保留前40+若干位（我取了72位）即可，这样可以降低大数运算的复杂度。[code](./uva12333-1.cpp)
